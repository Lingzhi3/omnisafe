...
def compute_advantages(pi_probs, qvalues, use_relu=False):
    ...
    advantages = qvalues.detach() - baseline            # todo 注意baseline维度，是否有广播机制？？
    if use_relu == True:
...
def compute_regrets(pi_probs, qvalues, use_relu=True):                  # 默认使用relu
    ...
    regrets = torch.sum(F.relu(qvalues.detach() - baseline), dim=1)      # qvalues:(batch,2) baseline:(batch,1) 广播？？
    return regrets
